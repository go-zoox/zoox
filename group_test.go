...
func TestGroupMiddlewareCache(t *testing.T) {
	app := New()
	...
	// Create a nested group with its own middleware
	v1 := api.Group("/v1")
	v1.Use(func(ctx *Context) {
	...
	// Test that middleware cache is initially invalid
	if api.middlewareCacheValid {
		t.Error("Expected middleware cache to be invalid initially")
	}

	// Get middleware cache - should trigger computation
	middlewares := api.getMiddlewareCache()
	if len(middlewares) == 0 {
		t.Error("Expected middleware cache to contain middlewares")
	}

	// Test that cache is now valid
	if !api.middlewareCacheValid {
		t.Error("Expected middleware cache to be valid after computation")
	}

	// Test that nested group inherits parent middlewares
	v1Middlewares := v1.getMiddlewareCache()
	if len(v1Middlewares) <= len(middlewares) {
		t.Error("Expected nested group to have more middlewares than parent")
	}

	// Test cache invalidation
	api.Use(func(ctx *Context) {
		ctx.Set("middleware3", "executed")
		ctx.Next()
	})

	// Parent cache should be invalidated
	if api.middlewareCacheValid {
		t.Error("Expected parent middleware cache to be invalidated after adding middleware")
	}

	// Child cache should also be invalidated
	if v1.middlewareCacheValid {
		t.Error("Expected child middleware cache to be invalidated after adding middleware to parent")
	}

	// Re-compute cache
	newMiddlewares := api.getMiddlewareCache()
	if len(newMiddlewares) != len(middlewares)+1 {
		t.Errorf("Expected %d middlewares, got %d", len(middlewares)+1, len(newMiddlewares))
	}
}
